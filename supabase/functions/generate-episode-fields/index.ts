...
serve(async (req) => {
  ...
  try {
    ...
    const { idea_principal, conflicto_central, intencion_del_episodio, tono, restricciones, episode_number } = await req.json();
...
{
  "working_title": "título de trabajo interno, formato Ep. ${episode_number || 'XX'} — [Título]. Máximo 7 palabras en el título. Usa arquetipo: verdad que nadie dice / pregunta que incomoda / costo invisible / inversión de expectativa.",
